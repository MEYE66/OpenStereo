#!/usr/bin/env python3
"""Batch RAFTStereo disparity visualization for CARLA 1280x384 AE methods."""
...
METHODS = ("pid", "semantic", "mixed", "gradient", "contrast", "exposure_agent", "drl")
WORKSPACE_ROOT = Path(__file__).resolve().parents[1]
DATA_ROOT = Path("/home/lgz/dataset/ADEC")
DATASET = "carla_1280x384"
DEFAULT_CFG = Path("cfgs/raftstereo/raftstereo_carla_1280x384_rgb.yaml")
DEFAULT_CKPT = Path(
    "./output/CarlaStereoDataset/RAFTStereo/"
    "raftstereo_carla_1280x384_rgb/default/ckpt/checkpoint_epoch_19.pth"
)
DEFAULT_OUTPUT_ROOT = Path("output_disparity") / DATASET
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run tools/infer.py for CARLA 1280x384 AE methods."
    )
...
def check_inputs(args: argparse.Namespace) -> None:
    ...
    for method in args.methods:
        val_txt = DATA_ROOT / DATASET / "ae_methods" / method / "val.txt"
...
def write_eval_cfg(run_dir: Path, method: str, batch_size: int) -> Path:
    val_txt = DATA_ROOT / DATASET / "ae_methods" / method / "val.txt"
    cfg_path = run_dir / "cfgs" / f"{DATASET}_{method}.yaml"
    ...
    cfg_path.write_text(
        "\n".join(
            [
                ...
                "    DATA_TRANSFORM:",
                "        EVALUATING:",
                "            - { NAME: RightTopPad, SIZE: [ 384, 1280 ] }",
                "            - { NAME: TransposeImage }",
                "            - { NAME: ToTensor }",
                "",
                "EVALUATOR:",
