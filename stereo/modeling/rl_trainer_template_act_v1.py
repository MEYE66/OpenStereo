...
class A2CTrainerTemplate(TrainerTemplate):
    ...
    def train_one_epoch(self, current_epoch, tbar):
        ...
        for i in range(0, len(self.train_loader)):
            ...
            action_abs_mean = torch.stack(
                [transition['action'].detach().abs().mean(dim=1) for transition in transitions],
                dim=0,
            ).mean().item()
            action_mean_abs = self._transition_mean(transitions, 'action_mean_abs')
            action_delta_abs = self._transition_mean(transitions, 'action_delta_abs')
            decoded_left_action_abs = self._transition_mean(transitions, 'decoded_left_action_abs')
            decoded_right_action_abs = self._transition_mean(transitions, 'decoded_right_action_abs')

            reward_disp_stats = self._transition_stats(transitions, 'reward_disp')
            reward_cover_stats = self._transition_stats(transitions, 'reward_cover')
            reward_disp_raw_stats = self._transition_stats(transitions, 'reward_disp_raw')
            reward_cover_raw_stats = self._transition_stats(transitions, 'reward_cover_raw')
            reward_total_raw_stats = self._transition_stats(transitions, 'reward_total_raw')
            reward_disp_mean = reward_disp_stats['mean'] if reward_disp_stats is not None else None
            reward_cover_mean = reward_cover_stats['mean'] if reward_cover_stats is not None else None
            cover_current_mean = self._transition_mean(transitions, 'cover_exposed_current')
            cover_next_mean = self._transition_mean(transitions, 'cover_exposed_next')
            ...
            action_log_std_mean = self._transition_mean(transitions, 'action_log_std')
            action_saturation_ratio = self._transition_mean(transitions, 'action_saturation')
            lce_amount_mean = self._transition_mean(transitions, 'lce_amount')
            lce_blur_mix_mean = self._transition_mean(transitions, 'lce_blur_mix')
            lce_threshold_mean = self._transition_mean(transitions, 'lce_threshold')

            coverage_message = ''
            if reward_disp_mean is not None and reward_cover_mean is not None and cover_next_mean is not None:
                coverage_message += ' RDisp:{:.4f} RCov:{:.4f} CovCur:{:.4f} CovNext:{:.4f}'.format(
                    reward_disp_mean,
                    reward_cover_mean,
                    cover_current_mean if cover_current_mean is not None else 0.0,
                    cover_next_mean,
                )
            if reward_disp_raw_stats is not None and reward_cover_raw_stats is not None:
                coverage_message += ' RDispRaw:{:.4f} RCovRaw:{:.4f}'.format(
                    reward_disp_raw_stats['mean'],
                    reward_cover_raw_stats['mean'],
                )
            if reward_total_raw_stats is not None:
                coverage_message += ' RTotRaw:{:.4f}'.format(reward_total_raw_stats['mean'])
            if (
            ...
            if reward_disp_stats is not None:
                coverage_message += ' |RDisp|:{:.4f} RDispStd:{:.4f} RDispMax:{:.4f}'.format(
                    reward_disp_stats['abs_mean'],
                    reward_disp_stats['std'],
                    reward_disp_stats['abs_max'],
                )
            if reward_cover_stats is not None:
                coverage_message += ' |RCov|:{:.4f} RCovStd:{:.4f} RCovMax:{:.4f}'.format(
                    reward_cover_stats['abs_mean'],
                    reward_cover_stats['std'],
                    reward_cover_stats['abs_max'],
                )
            if action_log_std_mean is not None:
                coverage_message += ' LogStd:{:.4f}'.format(action_log_std_mean)
            if action_saturation_ratio is not None:
                coverage_message += ' Sat:{:.4f}'.format(action_saturation_ratio)
            if action_mean_abs is not None and action_delta_abs is not None:
                coverage_message += ' ActM:{:.4f} ActD:{:.4f}'.format(action_mean_abs, action_delta_abs)
            if decoded_left_action_abs is not None and decoded_right_action_abs is not None:
                coverage_message += ' DecL:{:.4f} DecR:{:.4f}'.format(
                    decoded_left_action_abs,
                    decoded_right_action_abs,
                )
            if lce_amount_mean is not None and lce_blur_mix_mean is not None and lce_threshold_mean is not None:
                coverage_message += ' LCE(a/m/t):{:.4f}/{:.4f}/{:.4f}'.format(
                    lce_amount_mean,
                    lce_blur_mix_mean,
                    lce_threshold_mean,
                )
            ...
            tb_info.update({
                ...
                'scalar/train/action_abs_mean': action_abs_mean,
            ...
            if action_mean_abs is not None:
                tb_info['scalar/train/action_mean_abs'] = action_mean_abs
            if action_delta_abs is not None:
                tb_info['scalar/train/action_delta_abs'] = action_delta_abs
            if decoded_left_action_abs is not None:
                tb_info['scalar/train/decoded_left_action_abs'] = decoded_left_action_abs
            if decoded_right_action_abs is not None:
                tb_info['scalar/train/decoded_right_action_abs'] = decoded_right_action_abs

            if reward_disp_mean is not None:
                tb_info['scalar/train/reward_disp'] = reward_disp_mean
                tb_info['scalar/train/reward_disp_abs_mean'] = reward_disp_stats['abs_mean']
                tb_info['scalar/train/reward_disp_std'] = reward_disp_stats['std']
                tb_info['scalar/train/reward_disp_abs_max'] = reward_disp_stats['abs_max']
            if reward_disp_raw_stats is not None:
                tb_info['scalar/train/reward_disp_raw'] = reward_disp_raw_stats['mean']
            if reward_cover_mean is not None:
                tb_info['scalar/train/reward_cover'] = reward_cover_mean
                tb_info['scalar/train/reward_cover_abs_mean'] = reward_cover_stats['abs_mean']
                tb_info['scalar/train/reward_cover_std'] = reward_cover_stats['std']
                tb_info['scalar/train/reward_cover_abs_max'] = reward_cover_stats['abs_max']
            if reward_cover_raw_stats is not None:
                tb_info['scalar/train/reward_cover_raw'] = reward_cover_raw_stats['mean']
            if reward_total_raw_stats is not None:
                tb_info['scalar/train/reward_total_raw'] = reward_total_raw_stats['mean']
            if action_log_std_mean is not None:
                tb_info['scalar/train/action_log_std_mean'] = action_log_std_mean
                tb_info['scalar/train/log_std_mean'] = action_log_std_mean
            if action_saturation_ratio is not None:
                tb_info['scalar/train/action_saturation_ratio'] = action_saturation_ratio
            if cover_next_mean is not None:
                tb_info['scalar/train/cover_next'] = cover_next_mean
                tb_info['scalar/train/cover_exposed_next'] = cover_next_mean
            if cover_current_mean is not None:
                tb_info['scalar/train/cover_exposed_current'] = cover_current_mean
            ...
            if lce_amount_mean is not None:
                tb_info['scalar/train/lce_amount'] = lce_amount_mean
            if lce_blur_mix_mean is not None:
                tb_info['scalar/train/lce_blur_mix'] = lce_blur_mix_mean
            if lce_threshold_mean is not None:
                tb_info['scalar/train/lce_threshold'] = lce_threshold_mean
