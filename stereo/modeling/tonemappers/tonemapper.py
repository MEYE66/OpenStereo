...
from .utils import tanh_range, build_net
...
# Real-time Tone Mapping for Scene-Adaptive HDR Detecion
class SANet(nn.Module):  
    def __init__(self, layer=4, num_feat=32):
        ...
        self.image_adaptive_local = nn.Sequential(
            nn.Conv2d(3, num_feat, kernel_size=3, stride=1, padding=1, bias=False), # False 
            nn.ReLU(),
...
def thop_model_computation(model, input_data):
    ...
    print('{:<30}  {:<8}'.format('Computational complexity FLOPs(G): ', flops))
...
if __name__ == '__main__':
    ...
    model_computation(ianet, input_data)
    thop_model_computation(ianet, input_data)
    inference_latency_gpu(ianet, input_data)


    raodnet = RAODNet()
    model_computation(raodnet, input_data)
    thop_model_computation(raodnet, input_data)
