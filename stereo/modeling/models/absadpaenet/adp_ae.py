...
try:
    ...
    from stereo.modeling.models.absadpaenet.submodules import (
        A2CActor,
        AbsoluteExposureEnv,
        DispCritic,
...
class AbsAdpAENet(RAFTStereoDual):
    ...
    def _get_disp_target(self, batch):
        ...
        if disp_gt is None:
            raise KeyError('AbsAdpAENet requires disp or disp_1 in the batch.')
        if disp_gt.ndim == 4:
    ...
    def _require_dual_frame_batch(batch):
        ...
        if missing_keys:
            raise KeyError(
                'AbsAdpAENet requires dual-frame inputs. Missing keys: {}'.format(
                    ', '.join(missing_keys)
                )
            )
