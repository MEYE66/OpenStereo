...
try:
    corr_sampler = importlib.import_module('corr_sampler')
except ImportError:
    pass

try:
    alt_cuda_corr = importlib.import_module('alt_cuda_corr')
except ImportError:
...
class BasicEncoder(nn.Module):
    def __init__(self, output_dim=128, norm_fn='batch', dropout=0.0, downsample=3, num_input=3):
        ...
        self.conv1 = nn.Conv2d(num_input, 64, kernel_size=7, stride=1 + (downsample > 2), padding=3)
...
class CorrSampler(torch.autograd.Function):
    ...
    def forward(ctx, volume, coords, radius):
        if corr_sampler is None:
            raise ImportError('corr_sampler is not available. Use corr_implementation="reg" or build the CUDA extension.')
...
class AlternateCorrBlock:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4):
        if alt_cuda_corr is None:
            raise ImportError('alt_cuda_corr is not available. Use corr_implementation="alt" or build the CUDA extension.')
        self.num_levels = num_levels
        self.radius = radius
